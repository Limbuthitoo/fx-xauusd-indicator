...
- Module 2 research-only variants: `SWEEP_CLOSE_BACK_INSIDE`, `SWEEP_NO_CONFIRMATION`, `SWEEP_ENGULFING`, `SWEEP_BOS`, `SWEEP_MSS`, and `SWEEP_VOLUME_EXPANSION`. These can feed predictions, backtests, learning, and missed-trade reviews, but must not open paper trades by themselves.
- Module 2 paper-eligible variants: `SWEEP_DISPLACEMENT_RETEST`, `SWEEP_EMA_ALIGNMENT`, `SWEEP_BOS_RETEST`, `SWEEP_MSS_RETEST`, and `SWEEP_MSS_DISPLACEMENT_RETEST`.
- Module 2 paper trades require closed 5M candles, liquidity sweep + rejection, no acceptance beyond the swept level, displacement, protected structure confidence, BOS/MSS/CHoCH, fresh FVG/OB entry zone, retrace, entry confirmation candle, risk guardrails, selected paper-eligible variant, and Python brain approval.
...
- Module 2 tenant settings include EMA mode (`OFF`, `RECORD_ONLY`, `REQUIRE_ALIGNMENT`) and volume mode (`OFF`, `RECORD_ONLY`, `REQUIRE_EXPANSION`), plus NY premarket, ORB, and round-number controls.
- Module 2 state transitions are persisted in `module2_state_transitions` so sweep, displacement, BOS/CHoCH, entry-zone, retrace, entry-ready, invalidated, and expired evidence can be audited after the chart moves on.
