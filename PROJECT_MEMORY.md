...
- Live-chart paper-trade markers are secondary audit evidence, not BUY/SELL recommendations. They are hidden by default behind the `Paper audit` chart toggle; when enabled, an entry marker identifies its setup tier, confidence, entry, stop, target, and planned R so it cannot be confused with the primary strategy signal.
- Live-chart signal hierarchy is prediction-first: an incomplete current setup may show one amber `Watch BUY/SELL` marker only at 80%+ confidence; a completed actionable setup shows one explicit `BUY/SELL SIGNAL` marker and its immutable Entry, Structural SL, TP1, TP2, and TP3 geometry. Replay, rehearsal, QA, and production-proof trades are excluded from current plans and chart audit markers.
...
- Production signal quality policy: prioritize selective setups over signal frequency. A promoted XAUUSD BUY/SELL must have an evidence score of at least 80/100, directional structural geometry, TP1 at least 100 pips away using the stored XAUUSD pip size of 0.01, and a final target of at least 2R. The system must reject undersized plans rather than stretching an invalid target away from its structural stop.
- Production signal frequency policy: at most three distinct quality theses per New York trading date across all signal modules. There is no fixed Module 1/Module 2 slot allocation: either Module 1 strategy family or any independently approved Module 2 profile may qualify. A single strategy profile may contribute at most two signals and requires a 45-minute cooldown between distinct signals. Repeated five-minute confirmation of the same immutable thesis resumes its existing artifacts and never consumes another slot. Multiple Module 2 variants confirming the same sweep remain evidence for one thesis, not duplicate alerts. These are maximums, not a promise to fabricate a daily minimum when no quality setup exists.
- Production proof includes `npm run validate:signal-policy -- .env.production`. It audits promoted PostgreSQL contracts for immutable thesis deduplication, 80/100 evidence, minimum 100-pip TP1, minimum 2R final target, shared daily/profile limits, cooldown, BUY/SELL notification linkage, and secondary paper/journal tracking.
