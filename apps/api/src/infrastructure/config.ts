...
export const config = {
  ...
  twelveDataPollSeconds: Number(process.env.TWELVE_DATA_POLL_SECONDS ?? 300),
  twelveDataCatchupSeconds: Math.max(Number(process.env.TWELVE_DATA_CATCHUP_SECONDS ?? 300), 300),
  ...
  economicCalendarProvider: (process.env.ECONOMIC_CALENDAR_PROVIDER ?? "manual").trim().toLowerCase(),
  tradingEconomicsApiKey: process.env.TRADING_ECONOMICS_API_KEY ?? "",
  economicCalendarSyncMinutes: Math.max(Number(process.env.ECONOMIC_CALENDAR_SYNC_MINUTES ?? 360), 30),
  economicCalendarLookaheadDays: Math.min(Math.max(Number(process.env.ECONOMIC_CALENDAR_LOOKAHEAD_DAYS ?? 14), 2), 31),
  economicCalendarStaleHours: Math.max(Number(process.env.ECONOMIC_CALENDAR_STALE_HOURS ?? 30), 6),
  economicCalendarBlockBeforeMinutes: Math.min(Math.max(Number(process.env.ECONOMIC_CALENDAR_BLOCK_BEFORE_MINUTES ?? 30), 0), 240),
  economicCalendarBlockAfterMinutes: Math.min(Math.max(Number(process.env.ECONOMIC_CALENDAR_BLOCK_AFTER_MINUTES ?? 30), 0), 240),
