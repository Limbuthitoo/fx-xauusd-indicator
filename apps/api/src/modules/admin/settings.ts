...
export function validateModuleSetting(moduleCode: string, key: string, value: unknown) {
  if (moduleCode === "high_probability_strategy_2" && key === "liquiditySweep.strategy") {
    ...
    return {
      ...
      nyPremarketStartTime: timeValue(input.nyPremarketStartTime, "08:00"),
      orbStartTime: timeValue(input.orbStartTime, "09:30"),
      orbEndTime: timeValue(input.orbEndTime, "09:45"),
      ...
      minimumSweepDistanceATR: positiveNumber(input.minimumSweepDistanceATR, 0.02, 5),
      maximumSweepDistanceATR: positiveNumber(input.maximumSweepDistanceATR, 0.5, 10),
      ...
      minimumSwingProminenceATR: positiveNumber(input.minimumSwingProminenceATR, 0.2, 5),
      minimumBarsBetweenSwings: positiveInteger(input.minimumBarsBetweenSwings, 3, 20),
      structureToleranceATR: positiveNumber(input.structureToleranceATR, 0.03, 2),
      minimumBosCloseDistanceATR: positiveNumber(input.minimumBosCloseDistanceATR, 0.03, 2),
      maximumBarsAfterSweepForBos: positiveInteger(input.maximumBarsAfterSweepForBos, 10, 50),
      maximumBarsAfterBosForEntry: positiveInteger(input.maximumBarsAfterBosForEntry, 6, 50),
      ...
      liquidityReusePolicy: ["NEVER_REUSE", "REUSE_AFTER_RECLAIM", "REUSE_ON_NEW_SESSION", "MANUAL_REACTIVATION"].includes(String(input.liquidityReusePolicy)) ? String(input.liquidityReusePolicy) : "NEVER_REUSE",
      liquidityMergeToleranceATR: positiveNumber(input.liquidityMergeToleranceATR, 0.05, 2),
      maximumSwingLevelAgeDays: positiveInteger(input.maximumSwingLevelAgeDays, 5, 30),
      countertrendResolutionMode: ["RECORD_ONLY", "WARN", "BLOCK"].includes(String(input.countertrendResolutionMode)) ? String(input.countertrendResolutionMode) : "WARN",
      positionManagementMode: "FIXED_STOP_FIXED_TARGET",
      minimumTradesForInsight: positiveInteger(input.minimumTradesForInsight, 30, 500),
      emaFilterMode: ["OFF", "RECORD_ONLY", "WARN_ONLY", "REQUIRE_ALIGNMENT", "REQUIRE_COUNTERTREND"].includes(String(input.emaFilterMode)) ? String(input.emaFilterMode) : "WARN_ONLY",
      ...
      displacementFilterMode: ["OFF", "RECORD_ONLY", "WARN_ONLY", "REQUIRED"].includes(String(input.displacementFilterMode)) ? String(input.displacementFilterMode) : "WARN_ONLY",
      marketContextMode: ["OFF", "RECORD_ONLY", "WARN_ONLY", "REQUIRED"].includes(String(input.marketContextMode)) ? String(input.marketContextMode) : "RECORD_ONLY",
      manualConfirmationRequired: booleanValue(input.manualConfirmationRequired, false),
      maximumActiveSetupsPerSymbol: positiveInteger(input.maximumActiveSetupsPerSymbol, 1, 5),
      maximumActivePositions: positiveInteger(input.maximumActivePositions, 0, 5),
      riskPerTradePercent: positiveNumber(input.riskPerTradePercent, 0.25, 10),
      maximumDailyLossPercent: positiveNumber(input.maximumDailyLossPercent, 0.75, 25),
      maximumWeeklyLossPercent: positiveNumber(input.maximumWeeklyLossPercent, 2, 50),
      maximumConsecutiveLosses: positiveInteger(input.maximumConsecutiveLosses, 3, 20),
  ...
  return {
    ...
    risk: {
      ...
      atrPeriod: 14,
      minimumStopAtr: Math.max(2, positiveNumber(risk.minimumStopAtr, 2, 4)),
      liquidityBufferAtr: Math.max(0.25, positiveNumber(risk.liquidityBufferAtr, 0.25, 1)),
