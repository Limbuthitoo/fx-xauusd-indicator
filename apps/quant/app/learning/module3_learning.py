    ...
    with psycopg.connect(database_url, row_factory=dict_row) as conn:
        with conn.cursor() as cur:
            ...
            try:
                ...
                backtest_trades = load_backtest_trades(cur, tenant_id)
                ...
                recommendations = build_recommendations(all_trades, failures, indicator_rows)
                summary = build_summary(all_trades, failures, recommendations, indicator_rows, len(trades), len(backtest_trades))
                ...
                cur.execute(
                    ...
                    WHERE id = %s
                    """,
                    (len(all_trades), json.dumps(summary, default=str), run_id),
                )
                conn.commit()
                return {
                    "runId": str(run_id),
...
def load_backtest_trades(cur, tenant_id: str) -> list[dict[str, Any]]:
    cur.execute(
        """
        SELECT
          bt.id AS setup_id,
    ...
    for item in weakest:
        ...
        recommendations.append(
            recommendation(
                ...
                f"Improve Module 3 {item['indicator']} handling",
